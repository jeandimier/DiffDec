...
class FragmentReactionSliceEnumerator:
    ...
    def _scaffold_slicing(
        self, slice: FragmentedMolecule, cut: int
    ) -> Set[FragmentedMolecule]:
        to_add = set()
        if slice.decorations_count() == cut - 1:
            ...
            for pair in fragment_pairs:
                ...
                if scaffold:
                    ...
                    sliced_mol = self._create_sliced_molecule(
                        slice, labeled_scaffold, decoration
                    )
    ...
    def _select_all_except(self, fragments: Tuple[Mol], to_exclude: int) -> List[Mol]:
        return [
            fragment for indx, fragment in enumerate(fragments) if indx != to_exclude
        ]
    ...
    def _split_scaffold_from_decorations(
        self, pair: Tuple[Mol], cuts: int
    ) -> Tuple[Mol, Mol]:
        decoration = None
        scaffold = None
        for frag in pair:
