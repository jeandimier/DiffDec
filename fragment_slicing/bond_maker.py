...
class BondMaker:
    ...
    def join_scaffolds_and_decorations(
        ...
        decorations_smi = [
            ...
            for i, dec in enumerate(
                decorations_smi.split(self._tokens.ATTACHMENT_SEPARATOR_TOKEN)
            )
        ]
        num_attachment_points = len(
            self._attachment_points.get_attachment_points(scaffold_smi)
        )
    ...
    def join_molecule_fragments(
        self, scaffold: Mol, decoration: Mol, keep_label_on_atoms=False
    ):
        """
