...
class BondMapper:
    ...
    def convert_building_blocks_to_fragments(
        ...
        molecule: Mol,
        neighbor_map: Dict,
        list_of_outcomes: List[ReactionOutcomeDTO],
    ) -> List[Tuple[Mol]]:
        all_fragments = []
        ...
        for outcome in list_of_outcomes:
            try:
                ...
                for reagent_pair in reagent_pairs:
                    ...
                    bonds_to_cut = self._find_indices_of_target_bonds(
                        molecule, list_of_atom_pairs
                    )

                    if len(bonds_to_cut) == 1:
                        reaction_fragments = self._create_fragments(
                            molecule, bonds_to_cut
                        )
    ...
    def _indentify_mismatching_indices(
        self, original: Dict, derived: Dict
    ) -> List[Tuple]:
        def is_a_mismatch(original_points: List, derived_points: List):
    ...
    def _find_indices_of_target_bonds(
        self, molecule: Mol, list_of_atom_pairs: List[Tuple[int, int]]
    ) -> List[int]:
        list_of_atom_pairs = list(set(list_of_atom_pairs))
        bonds_to_cut = [
            molecule.GetBondBetweenAtoms(pair[0], pair[1]).GetIdx()
