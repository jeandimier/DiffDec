...
def main(args):
    ...
    sys.stdout = Logger(
        logpath=os.path.join(args.logs, "general_logs", experiment, f"log.log"),
        syspart=sys.stdout,
    )
    sys.stderr = Logger(
        logpath=os.path.join(args.logs, "general_logs", experiment, f"log.log"),
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--config", type=argparse.FileType(mode="r"), default="configs/single.yml"
    )
    ...
    p.add_argument(
        "--trainer_params",
        type=dict,
        help="parameters with keywords of the lightning trainer",
    )
    ...
    p.add_argument(
        "--datadir", type=str, default="/crossdock/", help="crossdock directory"
    )
    ...
    p.add_argument(
        "--wandb_entity", type=str, default="geometric", help="Entity (project) name"
    )
    p.add_argument(
        "--center_of_mass",
        type=str,
        default="scaffold",
        help="Where to center the data: scaffold | anchors",
    )
    p.add_argument(
        "--inpainting",
        action="store_true",
        default=False,
        help="Inpainting mode (full generation)",
    )
    p.add_argument(
        "--remove_anchors_context",
        action="store_true",
        default=False,
        help="Remove anchors context",
    )
