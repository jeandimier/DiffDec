...
def load_model_from_s3(
    s3_path: str, lightning_model_class: Type[pl.LightningModule], device="cpu"
) -> Type[pl.LightningDataModule]:
    """Loads a model from s3

    Args:
        s3_path (str): Full path of the model stored on s3
        lightning_model_class (Type[LightningModule]): The class of the model we want to
            load from s3. Has to inherit the LightningModule class.
        device (str, optional): The device on which to load the model. Defaults to "cpu".

    Returns:
        lightning_model_class: The loaded model.
    """
    with tempfile.NamedTemporaryFile() as tmp:
        s3.download(path=s3_path, local_file=tmp.name)
        model = lightning_model_class.load_from_checkpoint(tmp.name)
