...
def parse_rgroup_com_scaf(mol, fake_pos):
    ...
    for i in range(len(mol_frags)):
        ...
        positions = mol_frags[i].GetConformer().GetPositions()
        one_hot.extend(
            get_one_hot("#", atom2idx) for _ in range(10 - mol_frags[i].GetNumAtoms())
        )
        charges.extend(
            charges_dict["#"] for _ in range(10 - mol_frags[i].GetNumAtoms())
        )
        positions = positions.tolist()
...
def parse_rgroup(mol, fake_pos):
    if isinstance(fake_pos[0], list):
        ...
        for i in range(len(mol_frags)):
            ...
            positions = mol_frags[i].GetConformer().GetPositions()
            one_hot.extend(
                get_one_hot("#", atom2idx)
                for _ in range(10 - mol_frags[i].GetNumAtoms())
            )
            charges.extend(
                charges_dict["#"] for _ in range(10 - mol_frags[i].GetNumAtoms())
            )
            positions = positions.tolist()
            positions.extend(
                fake_pos[i] for _ in range(10 - mol_frags[i].GetNumAtoms())
            )
            one_hot_list.append(one_hot)
            charges_list.append(charges)
            positions_list.append(positions)
        ...
        positions = mol.GetConformer().GetPositions()
        one_hot.extend(
            get_one_hot("#", atom2idx) for _ in range(10 - mol.GetNumAtoms())
        )
...
def collate(batch):
    ...
    atom_mask = (out["scaffold_mask"].bool() | out["rgroup_mask"].bool()).to(
        const.TORCH_INT
    )
...
def collate_with_scaffold_edges(batch):
    ...
    diag_mask = ~torch.eye(
        edge_mask.size(1), dtype=const.TORCH_INT, device=scaf_mask.device
    ).unsqueeze(0)
    edge_mask *= diag_mask
    ...
    edges = [
        torch.LongTensor(rows).to(scaf_mask.device),
        torch.LongTensor(cols).to(scaf_mask.device),
    ]
    out["edges"] = edges
        ...
        for k, v in data.items():
            ...
            if k in const.DATA_ATTRS_TO_PAD:
                ...
                template = create_template(
                    v[i], scaffold_size, rgroup_size, fill=fill_value
                )
...
class CrossDockDataset(Dataset):
    def __init__(self, data_path, prefix, device):
        ...
        if os.path.exists(dataset_path):
            ...
            self.data = CrossDockDataset.preprocess(
                data_path, prefix, pocket_mode, device
            )
    ...
    def preprocess(data_path, prefix, pocket_mode, device):
        ...
        generator = tqdm(
            zip(
                table.iterrows(),
                read_sdf(scaffold_path),
                read_sdf(rgroups_path),
                pockets,
            ),
            total=len(table),
        )
        for (_, row), scaffold, rgroup, pocket_data in generator:
            ...
            uuid = row["uuid"]
            # cat = row['cat']
            name = row["molecule"]  # SMILES of the molecule, not its name
            ...
            one_hot = np.concatenate(
                [scaf_one_hot, pocket_one_hot, rgroup_one_hot], axis=0
            )
            charges = np.concatenate(
                [scaf_charges, pocket_charges, rgroup_charges], axis=0
            )
            ...
            data.append(
                {
                    ...
                    "scaffold_only_mask": torch.tensor(
                        scaf_only_mask, dtype=const.TORCH_FLOAT, device=device
                    ),
                    "pocket_mask": torch.tensor(
                        pocket_mask, dtype=const.TORCH_FLOAT, device=device
                    ),
                    "scaffold_mask": torch.tensor(
                        scaf_mask, dtype=const.TORCH_FLOAT, device=device
                    ),
                    "rgroup_mask": torch.tensor(
                        rgroup_mask, dtype=const.TORCH_FLOAT, device=device
                    ),
...
class CrossDockDatasetDivided(torch.utils.data.Dataset):
    """A dataset class that loads HeteroData objects from .pt files in a specified directory.

    The dataset scans a root directory for .pt files and returns them as individual samples
    suitable for use with a PyTorch Geometric DataLoader.
    """

    def __init__(self, root: str):
        """Initialize the dataset by collecting all .pt file paths in the given directory.

        Args:
            root (str): The path to the directory containing .pt files, potentially across subdirectories.
        """
        super().__init__()
        self.file_paths = []
        for subdir, _, files in os.walk(root):
            for f in files:
                if f.endswith(".pt"):
                    self.file_paths.append(os.path.join(subdir, f))

    def __len__(self) -> int:
        """Return the number of samples in the dataset.

        Returns:
            int: The total number of .pt files found under the specified root directory.
        """
        return len(self.file_paths)

    def __getitem__(self, idx: int) -> dict:
        """Load and return a single HeteroData object by index.

        Args:
            idx (int): The sample index to retrieve.

        Returns:
            HeteroData: The loaded PyTorch Geometric heterogeneous data object.
        """
        # load a HeteroData object
...
class MultiRDataset(Dataset):
    ...
    def preprocess(data_path, prefix, pocket_mode, device):
        ...
        for (_, row), scaffold, rgroup, pocket_data in generator:
            ...
            rgroup_pos, rgroup_one_hot, rgroup_charges = parse_rgroup_com_scaf(
                rgroup, fake_pos
            )
            # rgroup_pos, rgroup_one_hot, rgroup_charges = parse_molecule(rgroup)
...
class MultiRDataset_anchor(Dataset):
    def __init__(self, data_path, prefix, device):
        ...
        if os.path.exists(dataset_path):
            ...
            print(f"Preprocessing dataset with prefix {prefix}")
            self.data = MultiRDataset_anchor.preprocess(
                data_path, prefix, pocket_mode, device
            )
            torch.save(self.data, dataset_path)
    ...
    def preprocess(data_path, prefix, pocket_mode, device):
        ...
        for (_, row), scaffold, rgroup, pocket_data in generator:
            ...
            scaf_pos, scaf_one_hot, scaf_charges = parse_molecule(scaffold)
            fake_pos_list = [
                list(scaf_pos[int(anchor_id)]) for anchor_id in anchor_id_list
            ]
            try:
                rgroup_pos, rgroup_one_hot, rgroup_charges = parse_rgroup(
                    rgroup, fake_pos_list
                )
            except:
                continue
...
def collate_mr(batch):
    ...
    anchors_ = torch.zeros(
        [len(anchor_id_list), anchors.shape[1], anchors.shape[2]]
    ).to(rgroup_mask.device)
    ...
    rgroup_mask_batch_new = torch.zeros(
        [anchors_.shape[0], rgroup_mask.shape[1], rgroup_mask.shape[2]]
    ).to(rgroup_mask.device)
    cnt = 0
    for i in range(len(rgroup_size_list)):
        for j in range(len(rgroup_size_list[i])):
            rgroup_mask_batch_new[
                cnt,
                tmp_idx[cnt][1] : tmp_idx[cnt][1] + rgroup_size_list[i][j],
                tmp_idx[cnt][2],
            ] = 1
            cnt += 1
    out["rgroup_mask_batch_new"] = rgroup_mask_batch_new

    rgroup_mask_ori_batch_new = torch.repeat_interleave(
        rgroup_mask, batch_new_len_tensor, dim=0
    ).to(rgroup_mask.device)
    scaffold_mask_ori_batch_new = torch.repeat_interleave(
        scaffold_mask, batch_new_len_tensor, dim=0
    ).to(rgroup_mask.device)
    # edge_mask_batch_new = torch.repeat_interleave(edge_mask, batch_new_len_tensor, dim=0).to(rgroup_mask.device)
    node_mask_batch_new = torch.repeat_interleave(
        node_mask, batch_new_len_tensor, dim=0
    ).to(rgroup_mask.device)
    scaffold_mask_batch_new = scaffold_mask_ori_batch_new + (
        rgroup_mask_ori_batch_new - rgroup_mask_batch_new
    )
    ...
    pocket_mask_batch_new = torch.repeat_interleave(
        pocket_mask, batch_new_len_tensor, dim=0
    ).to(rgroup_mask.device)
    ...
    x_batch_new = torch.repeat_interleave(x, batch_new_len_tensor, dim=0).to(
        rgroup_mask.device
    )
    h_batch_new = torch.repeat_interleave(h, batch_new_len_tensor, dim=0).to(
        rgroup_mask.device
    )
