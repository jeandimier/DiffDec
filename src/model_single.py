...
from src.visualizer import save_xyz_file_fa, visualize_chain
...
class DDPM(pl.LightningModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        dataset_type = CrossDockDatasetDivided
        if stage == "fit":
            self.is_geom = True
            self.train_dataset = dataset_type(self.train_data_path.as_posix())
            self.train_dataset = torch.utils.data.Subset(
                self.train_dataset, list(range(0, 10000))
            )
            self.val_dataset = dataset_type(self.test_data_path.as_posix())
        elif stage == "val":
            self.is_geom = True
            self.val_dataset = dataset_type(self.test_data_path.as_posix())
        else:
    ...
    def train_dataloader(self, collate_fn=collate):
        return get_dataloader(
            self.train_dataset, self.batch_size, collate_fn=collate_fn, shuffle=True
        )
    ...
    def training_step(self, data, *args):
        ...
        if (
            ...
            and self.global_step % self.log_iterations == 0
        ):
    ...
    def on_validation_epoch_end(self):
        for metric in self.val_step_outputs[0].keys():
            avg_metric = self.aggregate_metric(self.val_step_outputs, metric)
    ...
    def generate_animation(self, chain_batch, node_mask, batch_i):
        batch_indices, mol_indices = utils.get_batch_idx_for_animation(
        ...
        for bi, mi in zip(batch_indices, mol_indices):
            ...
            chain_output = os.path.join(
                self.samples_dir, f"epoch_{self.current_epoch}", name
            )
            os.makedirs(chain_output, exist_ok=True)

            one_hot = chain[:, :, 3:-1] if self.include_charges else chain[:, :, 3:]
            positions = chain[:, :, :3]
            chain_node_mask = torch.cat(
                [node_mask[bi].unsqueeze(0) for _ in range(self.FRAMES)], dim=0
            )
            names = [f"{name}_{j}" for j in range(self.FRAMES)]

            save_xyz_file_fa(
                chain_output, one_hot, positions, chain_node_mask, names=names
            )
    ...
    def sample_chain(self, data, sample_fn=None, keep_frames=None):
        ...
        template_data = create_templates_for_rgroup_generation_single(
            data, rgroup_sizes
        )
    ...
    def configure_optimizers(self):
        return torch.optim.AdamW(
            self.edm.parameters(), lr=self.lr, amsgrad=True, weight_decay=1e-12
        )
