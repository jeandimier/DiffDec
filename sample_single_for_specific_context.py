...
def get_pocket(mol, pdb_path):
    ...
    distances = np.linalg.norm(
        atom_coords[:, None, :] - mol_atom_coords[None, :, :], axis=-1
    )
...
def find_correct_matches(matches_scaf, matches_rgroup, mol):
    non_intersecting_matches = find_non_intersecting_matches(
        matches_scaf, matches_rgroup
    )
...
def prepare(
    scaffold_smiles_file, protein_file, scaffold_file, task_name, data_dir, mode="test"
):
...
def sample(checkpoint, samples_dir, data_dir, n_samples, task_name, device):
    ...
    for batch_idx, data in enumerate(dataloader):
        ...
        for i in tqdm(range(starting_point, n_samples), desc=str(batch_idx)):
            chain, node_mask = model.sample_chain(
                data, sample_fn=sample_fn, keep_frames=1
            )
            ...
            for j in range(len(pred_names)):
                ...
                subprocess.run(
                    f"obabel {out_xyz} -O {out_sdf} 2> /dev/null", shell=True
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--scaffold_smiles_file", action="store", type=str, required=True
    )
